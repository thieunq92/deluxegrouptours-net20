...
namespace Portal.Modules.OrientalSails.Web.Admin
{
    public partial class SailsTripList : SailsAdminBase
    {
        ...
        {
            rptTripList.DataSource = Module.TripGetAll(false, null);
        ...
        protected void rptCruises_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                if (_currentTrip.HasCruise) e.Item.Visible = false;
                HyperLink hplCruise = e.Item.FindControl("hplCruise") as HyperLink;
                if (hplCruise != null)
                {
                    hplCruise.Text = _currentTrip.Name;
                    hplCruise.NavigateUrl =
                        string.Format(
                            "SailsPriceConfig.aspx?NodeId={0}&SectionId={1}&TripId={2}&Option={3}&cruiseid={4}",
                            Node.Id, Section.Id, _currentTrip.Id, _currentOption, 3);
                }
            }
        }
