...
namespace Portal.Modules.OrientalSails.Web.Admin
{
    public partial class ViewMeetings : SailsAdminBase
    {
        ...
        {
            if (IsUserAdministrator() || Module.PermissionCheck("VIEW_ALLBOOKINGRECEIVABLE", UserIdentity))
            {
        ...
        public IList<Activity> GetActivities(int pageSize, int pageIndex, out int count, bool salesFilter)
        {
            ...
            var dateFromCriterion = null as ICriterion;
            var dateToCriterion = null as ICriterion;
            var finalCriterion = Expression.Ge("Id", 0) as ICriterion;
            var sales = GetSalesOnQueryString();
            var salesCriterion = null as ICriterion;

            if (dateFrom != null)
            {
                var ts = new TimeSpan(0, 0, 0);
                dateFromCriterion = Expression.Ge("DateMeeting", dateFrom.Value.Date + ts);
                finalCriterion = Expression.And(finalCriterion, dateFromCriterion);
            }
            if (dateTo != null)
            {
                var ts = new TimeSpan(23, 59, 59);
                dateToCriterion = Expression.Le("DateMeeting", dateTo.Value.Date + ts);
                finalCriterion = Expression.And(finalCriterion, dateToCriterion);
            }


            if (IsUserAdministrator() || Module.PermissionCheck("VIEW_ALL_MEETING", UserIdentity))
            {
                if (salesFilter == true)
                {
                    if (sales != null)
        ...
        public void Redirect(string control)
        {
            ...
            nvc.Remove("sortut");
            nvc.Remove("sortdm");
            rawUrl = Request.Url.AbsolutePath + "?" + nvc.ToString();
            if (dateFrom != null)
        ...
        public void DisplaySalesColumn()
        {
            if (!IsUserAdministrator() || Module.PermissionCheck("VIEW_ALLBOOKINGRECEIVABLE", UserIdentity))
            {
                ...
                thSales.Visible = false;
                for (int i = 0; i < rptMeetings.Items.Count; i++)
                {
                    rptMeetings.Items[i].FindControl("tdSales").Visible = false;
                }
            }
        }
