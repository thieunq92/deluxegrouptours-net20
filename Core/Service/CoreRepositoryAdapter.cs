...
namespace CMS.Core.Service
{
	...
	public class CoreRepositoryAdapter : CoreRepository
	{
        ...
        public override IList GetSystemSettings()
        {
            return _commonDao.GetObjectByCriterion(typeof (SystemSetting), Expression.IsNull(SystemSetting.MODULETYPE));
        }

        public override IList GetModuleSettings(ModuleType moduleType)
        {
            return _commonDao.GetObjectByCriterion(typeof(SystemSetting), Expression.Eq(SystemSetting.MODULETYPE, moduleType));
        }
        #endregion
	}
}