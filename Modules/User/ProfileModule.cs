...
using User = CMS.Core.Domain.User;
	...
	public class ProfileModule : ModuleBase, IActionConsumer
	{
		...
		public void RegisterUser(string username, string emailAddress)
		{
			...
			catch
			{
				...
				CMS.Core.Domain.User newUser = this._userService.GetUserByUsernameAndEmail(username, emailAddress);
				if (newUser != null)
				{
		...
		public void UpdateUser(CMS.Core.Domain.User user)
		{
			// Save to database
		...
		/// <returns></returns>
		public CMS.Core.Domain.User GetUserById(int userId)
		{
